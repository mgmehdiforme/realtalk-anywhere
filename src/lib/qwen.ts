export interface QwenAnalysisResult {
  ...
  recommendedPhase:
    "Discover" | "Validate" | "Blueprint" | "Build" | "Launch" | "Scale" | "Partner";
...
const DEFAULT_MOCK_RESULT: QwenAnalysisResult = {
  ...
  biggestOpportunities: [
    {
      ...
      description:
        "You can validate the demand by offering manual scoping to 5 active pilot users before writing code.",
    },
    {
      title: "AI Scoping Differentiation",
      description:
        "Positioning the platform as a domain-specific scoped blueprint generator makes it stand out from generic AI agents.",
    },
  ],
  whatCouldSlowYouDown: [
    {
      ...
      businessImpact:
        "Increases cost and delays launch by several months, risking competitor entry.",
      technicalImpact:
        "Complex database relationships and API mocks created before core validation.",
    },
    {
      risk: "Sourcing Qualified Developers",
      businessImpact: "Slows down development pace and leads to communication gaps.",
      technicalImpact:
        "Technical debt accumulates if the initial code structure is not modular and well-documented.",
    },
  ],
  ifThisWereMyStartup: [
  ...
  recommendedPhaseReasoning:
    "Before committing resources to full engineering, we must validate that founders are willing to spend time filling out assessments to get these blueprints. Focus on customer problem confirmation.",
...
export async function analyzeBlueprintAnswers(
  ...
  if (!apiKey) {
    console.log(
      "No Qwen API Key found (QWEN_API_KEY / DASHSCOPE_API_KEY). Using mock report generator.",
    );
    ...
    if (timeline.includes("Less than 1 month")) {
      ...
      mock.recommendedPhaseReasoning =
        "With a critical launch timeline under 1 month, you must immediately enter the Build phase focusing exclusively on a single core feature and utilizing a pre-built SaaS starter kit.";
    } else if (answers.hasExistingCode === "yes_rescue") {
      mock.recommendedPhase = "Validate";
      mock.recommendedPhaseReasoning =
        "Given the existing codebase requires rescue or rewrite, we recommend a rapid technical audit and validation of the existing code structure to salvage what is clean before starting new development.";
    }
...
${JSON.stringify(answers, null, 2)}`;
  ...
  try {
    ...
    const fetchUrl = baseUrl.endsWith("/chat/completions")
      ? baseUrl
