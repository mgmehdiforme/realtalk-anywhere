...
import {
  ...
  exportAdminLeadsCsvAction,
} from "@/lib/admin-functions";
import type { BlogPost, BlogPostFaqItem, ArticleLeadRecord } from "@/lib/db";
...
function AdminBlogDashboard() {
  ...
  const [adminTab, setAdminTab] = useState<"articles" | "leads">("articles");
  const [leads, setLeads] = useState<ArticleLeadRecord[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const leadsData = await getAdminLeadsAction();
      if (leadsData.authenticated && leadsData.leads) {
  ...
  const handleAddFaq = () => {
    const currentFaqs = editFormData.faqs || [];
    setEditFormData({
      ...editFormData,
  ...
  const handleUpdateFaq = (index: number, field: "question" | "answer", val: string) => {
    ...
    if (currentFaqs[index]) {
      ...
      setEditFormData({ ...editFormData, faqs: currentFaqs });
    }
  };
  ...
  const handleDeleteLead = async (id: string) => {
    if (!confirm("Are you sure you want to delete this captured lead record?")) return;
    try {
      await deleteAdminLeadAction({ data: { id } });
  ...
  const handleExportCsv = async () => {
    try {
      const res = await exportAdminLeadsCsvAction();
      if (!res.csv) return;
      const blob = new Blob([res.csv], { type: "text/csv;charset=utf-8;" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `captured-founder-leads-${new Date().toISOString().split("T")[0]}.csv`;
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="mx-auto max-w-7xl px-5 pt-8 sm:px-8 space-y-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
            <div className="flex items-center justify-between text-muted-foreground">
              <span className="text-xs font-semibold uppercase tracking-wider">Reading Vol</span>
              <Clock className="h-4 w-4 text-neon" />
            </div>
            <div className="mt-2 text-2xl font-bold font-display">
              {state?.metrics.totalReadMinutes || 0} min
            </div>
            <div className="text-[11px] text-muted-foreground mt-1">Combined reading time</div>
          </div>

          <div className="rounded-2xl border border-neon/30 bg-neon/5 p-5 shadow-card">
            <div className="flex items-center justify-between text-neon">
              <span className="text-xs font-semibold uppercase tracking-wider">Captured Leads</span>
              <Users className="h-4 w-4" />
            </div>
            <div className="mt-2 text-2xl font-bold font-display text-neon">
              {leads.length}
            </div>
            <div className="text-[11px] text-muted-foreground mt-1">Brief downloads & contacts</div>
          </div>
        </div>
                      ...
                      filteredPosts.map((post) => (
                        <tr key={post.id} className="hover:bg-muted/30 transition">
                          <td className="px-5 py-4">
                            <div className="font-semibold text-foreground max-w-sm sm:max-w-md line-clamp-1">
                              {post.title}
                            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  placeholder="Filter leads by founder name, email, or article..."
                  className="w-full rounded-xl border border-border bg-card pl-10 pr-4 py-2 text-xs text-foreground focus:border-neon focus:outline-none"
                />
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl border border-neon/40 bg-neon/10 px-4 py-2 text-xs font-bold text-neon hover:bg-neon hover:text-primary-foreground transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden shadow-card">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead className="border-b border-border bg-muted/40 font-semibold text-muted-foreground uppercase text-[10px] tracking-wider">
                    <tr>
                      <th className="px-5 py-3.5">Founder / Contact</th>
                      <th className="px-5 py-3.5">Role</th>
                      <th className="px-5 py-3.5">Article / Brief Downloaded</th>
                      <th className="px-5 py-3.5">Pillar</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {leads
                      .filter((l) => {
                        ...
                        return (
                          ...
                          l.articleTitle.toLowerCase().includes(s) ||
                        ...
                        <tr key={lead.id} className="hover:bg-muted/30 transition">
                          <td className="px-5 py-4">
                            ...
                            <a
                              ...
                              className="text-[11px] text-neon hover:underline inline-flex items-center gap-1 font-mono mt-0.5"
                            >
                          ...
                          <td className="px-5 py-4">
                            <span className="rounded-md border border-border bg-muted/50 px-2 py-1 text-[11px] font-medium text-foreground">
                              {lead.role || "Founder / CEO"}
                            </span>
                          </td>
                          <td className="px-5 py-4 max-w-xs">
                            <Link
                              to="/blog/$slug"
                              params={{ slug: lead.articleSlug }}
                              target="_blank"
                              className="font-medium text-foreground hover:text-neon line-clamp-1 transition"
                            >
                              {lead.articleTitle}
                            </Link>
                            <span className="text-[10px] text-muted-foreground font-mono">
                              /blog/{lead.articleSlug}
                            </span>
                          </td>
                          <td className="px-5 py-4">
                            <span className="rounded bg-neon/10 px-2 py-0.5 text-[10px] font-mono font-semibold text-neon uppercase">
                              {lead.pillar || "architecture"}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-muted-foreground text-[11px]">
                          ...
                          <td className="px-5 py-4 text-right">
                            <button
                              ...
                              className="rounded-lg p-1.5 text-destructive/70 hover:text-destructive hover:bg-destructive/10 transition cursor-pointer"
                    ...
                    {leads.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-5 py-12 text-center text-muted-foreground space-y-2">
                          <Users className="h-8 w-8 mx-auto text-neon/40" />
                          <div className="font-semibold text-foreground">No leads captured yet</div>
                          <p className="text-xs max-w-sm mx-auto">
                            Leads will appear here automatically as founders and CTOs download 2-page
                            executive architecture briefs from your blog guides.
                          </p>
                        </td>
                      </tr>
                    )}
          ...
          <div className="w-full max-w-5xl rounded-3xl border border-border bg-card shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {editTab === "edit" ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <label className="text-xs font-semibold text-foreground/90">
                          Frequently Asked Questions (Google Rich Snippets &amp; AI Search)
                        </label>
                      </div>
                      <button
                        type="button"
                        onClick={handleAddFaq}
                        className="inline-flex items-center gap-1 rounded-lg bg-neon/15 px-2.5 py-1 text-[11px] font-semibold text-neon border border-neon/30 hover:bg-neon hover:text-primary-foreground transition cursor-pointer"
                      >
                        <Plus className="h-3 w-3" /> Add FAQ
                      </button>
                    </div>

                    {!editFormData.faqs || editFormData.faqs.length === 0 ? (
                      <p className="text-[11px] text-muted-foreground italic">
                        No FAQs defined yet. Click "Add FAQ" to provide structured Q&amp;A for this article.
                      </p>
                    ) : (
                      <div className="space-y-3">
                        {editFormData.faqs.map((faq, idx) => (
                          <div
                            key={idx}
                            className="rounded-xl border border-border bg-background/80 p-3.5 space-y-2.5 relative group"
                          >
                            <div className="flex items-center justify-between gap-2">
                              <span className="font-mono text-[10px] font-bold text-neon uppercase">
                                FAQ #{idx + 1}
                              </span>
                              <button
                                type="button"
                                onClick={() => handleRemoveFaq(idx)}
                                className="rounded p-1 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition cursor-pointer"
                                title="Remove FAQ"
                              >
                            ...
                            <input
                              type="text"
                              placeholder="Question (e.g. When should a founder hire a Fractional CTO?)"
                              value={faq.question}
                              onChange={(e) => handleUpdateFaq(idx, "question", e.target.value)}
                              className="w-full rounded-lg border border-border bg-card px-3 py-1.5 text-xs text-foreground placeholder:text-muted-foreground focus:border-neon focus:outline-none"
                            />
                            <textarea
                              rows={2}
                              placeholder="Direct, authoritative answer (2-3 sentences explaining the trade-off or solution)..."
                              value={faq.answer}
                              onChange={(e) => handleUpdateFaq(idx, "answer", e.target.value)}
                              className="w-full rounded-lg border border-border bg-card p-2.5 text-xs text-foreground placeholder:text-muted-foreground focus:border-neon focus:outline-none"
                            />
                          </div>
                        ))}
                      </div>
                ...
                <div className="rounded-xl border border-border bg-background p-6 max-h-[50vh] overflow-y-auto space-y-6">
                  <div
                    ref={adminPreviewRef}
                  ...
                  {editFormData.faqs && editFormData.faqs.length > 0 && (
                    <div className="pt-6 border-t border-border space-y-3">
                      <h4 className="font-display font-bold text-sm text-foreground flex items-center gap-2">
                        <HelpCircle className="h-4 w-4 text-neon" /> Frequently Asked Questions
                      </h4>
                      <div className="space-y-2">
                        {editFormData.faqs.map((faq, i) => (
                          <div key={i} className="p-3 rounded-xl border border-border bg-card/60 space-y-1">
                            <p className="font-bold text-xs text-foreground">{faq.question}</p>
                            <p className="text-[11px] text-muted-foreground">{faq.answer}</p>
                          </div>
                        ))}
                      </div>
                    </div>
